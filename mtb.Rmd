...
pacman::p_load(tidyverse, tximport, biomaRt, DESeq2, WGCNA)
```
...
m <- lm(abundance ~ background*time_treat, data = metadata #|> filter(background != "NLRP3")
         )
 
m |> summary()
 
emmip(m, background ~ time_treat, CIs = TRUE) + ylab("expression") + theme(axis.title.x = element_blank())
```
...
deg <- DESeqDataSetFromMatrix(
  ...
  design = ~ background + time_treat + background:time_treat
  )

model.matrix(object = ~ background + time_treat + background:time_treat, 
